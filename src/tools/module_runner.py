...
class ModuleRunner(object):
    ...
    def _make_parallel(self, net):
        ...
        if self.configer.get('gpu') is not None and len(self.configer.get('gpu')) == 1:
            self.configer.update(['network', 'gathered'], True)

        return net  # DataParallelModel(net, gather_=self.configer.get('network', 'gathered'))
    ...
    def save_net(self, net, save_mode='iters'):
        ...
        state = {'config_dict': self.configer.to_dict(), 'state_dict': net.state_dict()}

        checkpoints_dir = self.configer.get('sv_path')

        torch.save(state, os.path.join(checkpoints_dir, 'latest.pth'))

        if save_mode == 'performance':
            if self.configer.get('performance') > self.configer.get('max_performance'):
                torch.save(state, os.path.join(checkpoints_dir, 'max_performance.pth'))
            ...
            if self.configer.get('val_loss') < self.configer.get('min_val_loss'):
                torch.save(state, os.path.join(checkpoints_dir, 'min_loss.pth'))
                self.configer.update(['min_val_loss'], self.configer.get('val_loss'))

        elif save_mode == 'epoch':
            if self.configer.get('epoch') - self.configer.get('last_epoch') >= 1:
                fname = 'epoch{}.pth'.format(self.configer.get('epoch'))
                torch.save(state, os.path.join(checkpoints_dir, fname))
