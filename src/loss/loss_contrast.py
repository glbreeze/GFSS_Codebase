...
class PixelContrastLoss(nn.Module, ABC):
    ...
    def _hard_anchor_sampling(self, X, y_hat, y): # y is pred, y_hat is GT
    ...
    def _contrastive(self, feats_, labels_):
        ...
        neg_logits = torch.exp(logits) * neg_mask               # 每一行中 与 当前 pixel 不同的 负类
...
class ContrastCELoss(nn.Module):
    ...
    def forward(self, logits, target, embedding=None, with_embed=True):
        ...
        if embedding is not None:
            ...
            pred = pred.argmax(1)
            loss_contrast = self.contrast_criterion(embedding, target, pred)   # only target is [473, 473]
