...
class FSCELoss(nn.Module):
    def __init__(self, configer=None, num_classes=None):
    ...
    def forward(self, logits, targets, weights=None,):
        """
            inputs:  logits : shape [batch_size, num_classes, h, w]
                     targets : shape [batch_size, h, w]
            returns: loss: shape []
            """
        ...
        if logits.shape[-2:] != targets.shape[-2:]:
            logits = F.interpolate(logits, size=targets.shape[-2:], mode='bilinear', align_corners=True)
...
class DiceLoss(nn.Module):
    ...
    def forward(self, logits, gt, eps=1e-7):
...
class WTCELoss(nn.Module):
    def __init__(self, configer=None, num_cls=2, fg_idx=1):   # loss_type ['wt_ce', 'wt_dc']
        super().__init__()
        self.loss_type = configer.get('loss', 'params').get('ce_weight', False)
